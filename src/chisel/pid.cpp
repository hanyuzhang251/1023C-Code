...
namespace chisel {
    ...
    PIDController::PIDController(const float kp, const float ki, const float kd, const float tolerance,
            const float wind, const float clamp, const float slew, const float small_error, const float large_error): kp(kp), ki(ki), kd(kd), tolerance(tolerance),
    ...
    void PIDController::update() {
        ...
        // Clamp the integral.
        integral = clamp_(integral, -clamp, clamp);
        ...
        // Clamp power to slew.
        calc_power = clamp_(calc_power, prev_output - slew, prev_output + slew);
